...
def get_stats():
    ...
    try:
        stats = {
            ...
            "critical_vulnerabilities": db.query(Vulnerability)
            .filter_by(severity=Severity.CRITICAL)
            .count(),
            "unacknowledged_alerts": db.query(Alert)
            .filter_by(acknowledged=False)
            .count(),
            "recent_scans": db.query(Scan)
            .filter(Scan.started_at > datetime.utcnow() - timedelta(hours=24))
            .count(),
        }
            ...
            [
                {
                    ...
                    "vulnerability_count": sum(
                        len(s.vulnerabilities) for s in d.services
                    ),
                }
                for d in devices
            ]
...
def get_vulnerabilities():
    ...
    try:
        ...
        vulnerabilities = (
            query.order_by(desc(Vulnerability.discovered_at)).limit(limit).all()
        )
            ...
            [
                {
                    ...
                    "service": {"port": v.service.port, "name": v.service.service_name},
                }
...
def get_scans():
    ...
    try:
        scans = db.query(Scan).order_by(desc(Scan.started_at)).limit(20).all()
