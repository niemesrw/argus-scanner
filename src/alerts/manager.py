...
class AlertManager:
    ...
    def send_critical_vulnerability_alert(self, device: Device, vulnerabilities: List[Dict]):
        """Send alert for critical vulnerabilities"""
        title = f"Critical Vulnerabilities Found on {device.hostname or device.ip_address}"
    ...
    def _send_email_notification(self, alert: Alert):
        ...
        body = f"{severity_emoji.get(alert.severity, '')} {alert.title}\n\n{alert.message}"
        msg.attach(MIMEText(body, 'plain'))
        
        # Send email
        with smtplib.SMTP(
            self.settings.alert_email_smtp_host,
            self.settings.alert_email_smtp_port
        ) as server:
            if self.settings.alert_email_username and self.settings.alert_email_password:
                server.starttls()
                server.login(
