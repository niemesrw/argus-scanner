...
class SchedulerService:
        ...
        try:
            ...
            for device_info in discovered_devices:
                device = (
                    ...
                    .filter_by(ip_address=device_info["ip_address"])
                    .first()
                )
            ...
            self.db.commit()
            logger.info(
                f"Network discovery completed. Found {len(discovered_devices)} devices"
            )
    ...
    def run_vulnerability_scan(self):
        ...
        try:
            ...
            self.db.commit()
            logger.info(
                f"Vulnerability scan completed. Found {vulnerabilities_found} vulnerabilities"
            )
    ...
    def cleanup_inactive_devices(self):
        ...
        try:
            ...
            inactive_devices = (
                self.db.query(Device)
                .filter(Device.last_seen < cutoff_date, Device.is_active == True)
                .all()
            )
    ...
    def _check_new_devices(self, discovered_devices: list):
        ...
        for device_info in discovered_devices:
            device = (
                self.db.query(Device)
                .filter_by(mac_address=device_info.get("mac_address"))
                .first()
            )
