...
class Settings:
    ...
    def __init__(self):
        ...
        self.vulnerability_db_update_interval = int(
            os.getenv("ARGUS_VULN_DB_UPDATE_INTERVAL", "86400")
        )

        # Alerting
        self.alert_email_enabled = (
            os.getenv("ARGUS_ALERT_EMAIL_ENABLED", "false").lower() == "true"
        )
        ...
        self.alert_email_to = (
            os.getenv("ARGUS_ALERT_TO", "").split(",")
            if os.getenv("ARGUS_ALERT_TO")
            else []
        )

        self.alert_slack_enabled = (
            os.getenv("ARGUS_ALERT_SLACK_ENABLED", "false").lower() == "true"
        )
        self.alert_slack_webhook = os.getenv("ARGUS_SLACK_WEBHOOK")

        # Security
        self.enable_exploit_testing = (
            os.getenv("ARGUS_ENABLE_EXPLOIT_TESTING", "false").lower() == "true"
        )
        self.authorized_networks = (
            os.getenv("ARGUS_AUTHORIZED_NETWORKS", "").split(",")
            if os.getenv("ARGUS_AUTHORIZED_NETWORKS")
            else []
        )
