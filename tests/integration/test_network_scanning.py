...
class TestNetworkScanningIntegration:
    ...
    def test_localhost_discovery(
        self, integration_settings, db_session, controlled_network_target
    ):
        ...
        localhost_found = any(device["ip"] == "127.0.0.1" for device in devices)
        assert localhost_found, "Localhost should be discovered"

        # Verify device was saved to database
        saved_device = (
            db_session.query(Device).filter_by(ip_address="127.0.0.1").first()
        )
    ...
    def test_port_scanning_integration(
        ...
        devices = discovery.discover_devices(controlled_network_target["network"])
        localhost_device = next((d for d in devices if d["ip"] == "127.0.0.1"), None)
        assert localhost_device is not None

        # Perform detailed scan with port detection
        detailed_info = discovery.scan_device_ports(
            localhost_device["ip"], controlled_network_target["safe_ports"]
        )
    ...
    def test_vulnerability_scanning_integration(self, integration_settings, db_session):
        ...
        if vulnerabilities:
            saved_vulns = (
                ...
                .filter_by(device_id=test_device.id)
                .all()
            )
    ...
    def test_network_interface_detection(self, integration_settings):
        ...
        lo_interface = next(
            (iface for iface in interfaces if iface["name"] == "lo"), None
        )
    ...
    def test_scan_timeout_handling(
        self, integration_settings, controlled_network_target
    ):
