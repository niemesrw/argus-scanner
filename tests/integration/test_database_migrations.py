...
class TestDatabaseMigrations:
    ...
    def test_database_schema_validation(self, temp_database):
        ...
        for col_name in required_device_cols:
            assert (
                col_name in device_columns
            ), f"Device table missing column: {col_name}"

        # Test Vulnerability table schema
        vuln_columns = {
            col["name"]: col for col in inspector.get_columns("vulnerabilities")
        }
        required_vuln_cols = [
            "id",
            "device_id",
        ...
        for col_name in required_vuln_cols:
            assert (
                col_name in vuln_columns
            ), f"Vulnerability table missing column: {col_name}"
    ...
    def test_foreign_key_constraints(self, temp_database):
        ...
        try:
            ...
            retrieved_vuln = (
                session.query(Vulnerability).filter_by(id=vulnerability.id).first()
            )
    ...
    def test_json_field_storage_retrieval(self, temp_database):
        ...
        try:
            ...
            metadata = {
                ...
                "os_details": {"kernel": "5.4.0", "architecture": "x86_64"},
            }
            ...
            retrieved_device = (
                session.query(Device).filter_by(mac_address="00:11:22:33:44:66").first()
            )
    ...
    def test_enum_field_storage(self, temp_database):
        ...
        try:
            ...
            retrieved_vuln = (
                session.query(Vulnerability).filter_by(cve_id="CVE-2023-54321").first()
            )
