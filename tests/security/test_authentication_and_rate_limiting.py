...
class TestAuthenticationSecurity:
    ...
    def test_api_endpoint_authentication(self, security_client):
        ...
        for endpoint in sensitive_endpoints:
            # Test GET request
            response = security_client.get(endpoint)
            assert response.status_code in [401, 403, 404, 405], f"Endpoint {endpoint} should require auth or not exist"
    ...
    def test_session_management(self, security_client):
        ...
        if session_id_1 and session_id_2 and response2.status_code == 200:
            assert session_id_1 != session_id_2, "Session ID should change after successful login"
    ...
    def test_password_policy_enforcement(self, security_client):
        ...
        for weak_password in weak_passwords:
            ...
            assert response.status_code in [400, 401, 403, 404, 422], f"Should reject weak password: {weak_password}"
    ...
    def test_jwt_token_security(self, security_client):
        ...
        for token in malformed_tokens:
            ...
            assert response.status_code in [401, 403, 404], f"Should reject malformed token: {token[:20]}..."
    ...
    def test_api_key_security(self, security_client):
        ...
        for api_key in invalid_keys:
            ...
            assert response.status_code in [200, 401, 403, 404]  # 200 if no auth required
    ...
    def test_cors_security(self, security_client):
        ...
        for origin in malicious_origins:
            ...
            if cors_header:
                ...
                assert cors_header != '*' or response.status_code != 200, "Should not allow wildcard CORS for sensitive endpoints"
                assert cors_header != origin, f"Should not reflect malicious origin: {origin}"
        ...
        for endpoint in info_endpoints:
            ...
            if response.status_code == 200:
                ...
                for keyword in sensitive_keywords:
                    assert keyword.lower() not in response_text.lower(), f"Endpoint {endpoint} exposes sensitive info: {keyword}"
...
class TestRateLimitingSecurity:
    ...
    def test_api_rate_limiting(self, security_client):
        ...
        for endpoint in endpoints_to_test:
            ...
            assert all(code in [200, 429, 404, 500] for code in status_codes), f"Unexpected status codes for {endpoint}: {status_codes}"
    ...
    def test_concurrent_request_handling(self, security_client):
        ...
        error_count = sum(1 for code in response_codes if isinstance(code, str) and 'Error' in code)
        assert error_count <= len(response_codes) // 2, "Too many errors in concurrent requests"
    ...
    def test_large_payload_protection(self, security_client):
        ...
        for size in payload_sizes:
            ...
            if size > 100000:  # For very large payloads
                assert response.status_code in [400, 413, 422], f"Should reject payload of size {size}"
            else:
    ...
    def test_resource_exhaustion_protection(self, security_client):
        ...
        for operation in operations:
            ...
            assert request_time < 10.0 or response.status_code >= 400, f"Operation {operation} took too long or should be rejected"
