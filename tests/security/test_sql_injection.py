...
class TestSQLInjectionPrevention:
    ...
    def test_device_search_sql_injection(self, security_client, security_db_session, malicious_payloads):
        ...
        for payload in malicious_payloads['sql_injection']:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, dict) and 'devices' in data:
                    ...
                    assert len(data['devices']) <= 1  # Should only match our test device at most
    
    def test_device_filter_sql_injection(self, security_client, security_db_session, malicious_payloads):
    ...
    def test_vulnerability_search_sql_injection(self, security_client, security_db_session, malicious_payloads):
    ...
    def test_alert_filter_sql_injection(self, security_client, security_db_session, malicious_payloads):
    ...
    def test_raw_sql_injection_protection(self, security_db_session, malicious_payloads):
        ...
        for payload in malicious_payloads['sql_injection']:
            try:
                ...
                result = security_db_session.query(Device).filter(
                    Device.hostname == payload
                ).first()
    ...
    def test_orm_injection_protection(self, security_db_session, malicious_payloads):
        ...
        for payload in malicious_payloads['sql_injection']:
            try:
                ...
                devices = security_db_session.query(Device).filter(
                    Device.ip_address.like(f'%{payload}%')
                ).all()
                
                assert isinstance(devices, list)
                
                # Test count operations
                count = security_db_session.query(Device).filter(
                    Device.hostname.contains(payload)
                ).count()
    ...
    def test_parameterized_query_usage(self, security_db_session):
        ...
        for test_input in special_inputs:
            ...
            try:
                ...
                # Test retrieval
                retrieved = security_db_session.query(Device).filter(
    ...
    def test_time_based_injection_protection(self, security_client, malicious_payloads):
        ...
        for payload in time_based_payloads:
            ...
            assert response_time < 2.0, f"Response took too long: {response_time}s for payload: {payload}"
            assert response.status_code in [200, 400, 422]
    
    def test_union_based_injection_protection(self, security_client, security_db_session, malicious_payloads):
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, dict) and 'devices' in data:
                    for device in data['devices']:
                        ...
                        expected_keys = {'id', 'mac_address', 'ip_address', 'hostname', 'first_seen', 'last_seen'}
                        actual_keys = set(device.keys())
                        
                        # Should not have extra columns from injection
                        assert not (actual_keys - expected_keys), f"Unexpected keys found: {actual_keys - expected_keys}"
    ...
    def test_blind_injection_protection(self, security_client):
        ...
        if len(set(response_lengths)) > 1:
            ...
            assert (max_length - min_length) / max_length < 0.5, "Response lengths vary too much"