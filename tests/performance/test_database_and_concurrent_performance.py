...
class TestDatabaseQueryOptimization:
    ...
    def test_device_query_performance_with_large_dataset(
        self, performance_db_session, large_dataset_factory, performance_timer
    ):
        ...
        queries = [
            ...
            lambda: performance_db_session.query(Device)
            .filter(Device.ip_address == "192.168.1.100")
            .first(),
            lambda: performance_db_session.query(Device)
            .filter(Device.mac_address.like("00:11:22%"))
            .all(),
            # Test range queries
            lambda: performance_db_session.query(Device)
            .filter(Device.ip_address.between("192.168.1.1", "192.168.1.255"))
            .all(),
            ...
            lambda: performance_db_session.query(Device)
            .filter(Device.device_type == "server")
            .filter(Device.hostname.contains("device"))
            .all(),
        ]
        ...
        for result in query_results:
            print(
                f"{result['query_index']:<10} {result['time']:<10.3f} {result['result_count']:<10}"
            )
    ...
    def test_vulnerability_aggregation_performance(
        self, performance_db_session, large_dataset_factory, performance_timer
    ):
        ...
        vulnerabilities = large_dataset_factory["vulnerabilities"](
            len(devices), 10
        )  # 10 vulns per device
        vuln_objects = []
        ...
        aggregation_queries = [
            ...
            lambda: performance_db_session.query(
            ...
            lambda: performance_db_session.query(
                Device.hostname, func.count(Vulnerability.id)
            )
            .join(Vulnerability)
            .group_by(Device.id)
            .all(),
        ...
        for i, query in enumerate(aggregation_queries):
            ...
            assert (
                query_time < 5.0
            ), f"Aggregation query {i} took too long: {query_time:.3f}s"

            print(f"Aggregation query {i}: {query_time:.3f}s")

    def test_database_indexing_effectiveness(
        self, performance_db_session, large_dataset_factory, performance_timer
    ):
        ...
        indexed_queries = [
            ...
            lambda: performance_db_session.query(Device)
            .filter(Device.mac_address == "00:11:22:33:44:55")
            .first(),
        ]
    ...
    def test_concurrent_database_access(self, performance_settings, performance_timer):
        ...
        assert (
            len(successful_times) >= 7
        ), f"Too many database errors in concurrent access: {len(errors)} errors"
        ...
        if successful_times:
            ...
            assert (
                avg_time < 5.0
            ), f"Concurrent database operations too slow: {avg_time:.3f}s"

            print(
                f"Concurrent database access: {len(successful_times)} successful, {len(errors)} errors"
            )
...
class TestConcurrentScanPerformance:
    ...
    def test_concurrent_network_discovery(
        self, performance_settings, performance_timer
    ):
        ...
        def discovery_task(network_range, task_id):
            ...
            with patch.object(discovery.nm, "scan") as mock_scan, patch.object(
                discovery.nm, "all_hosts"
            ) as mock_hosts:

                # Simulate different number of hosts per range
                host_count = 10 + (task_id % 5)  # 10-14 hosts per range
                mock_hosts.return_value = [
                    f"192.168.{task_id}.{i}" for i in range(1, host_count + 1)
                ]
    ...
    def test_concurrent_vulnerability_scanning(
        ...
        assert (
            total_time < 45.0
        ), f"Concurrent vulnerability scanning took too long: {total_time:.3f}s"
        assert total_devices_scanned == len(devices), "Should scan all devices"
        assert (
            total_vulnerabilities >= 0
        ), "Should find vulnerabilities or handle gracefully"
    ...
    def test_concurrent_scheduler_tasks(self, performance_settings, performance_timer):
        ...
        def scheduler_task(task_type, task_id):
            ...
            try:
                if task_type == "discovery":
                    # Mock discovery task
                    with patch.object(
                        NetworkDiscovery, "discover_devices"
                    ) as mock_discover:
                    ...
                    with patch.object(
                        VulnerabilityScanner, "scan_all_devices"
                    ) as mock_scan:
        ...
        if successful_tasks:
            avg_task_time = sum(r["time"] for r in successful_tasks) / len(
                successful_tasks
            )
            print(f"  Average task time: {avg_task_time:.3f}s")
        ...
        assert (
            total_time < 60.0
        ), f"Concurrent scheduler tasks took too long: {total_time:.3f}s"
        assert (
            len(successful_tasks) >= len(tasks) * 0.8
...
class TestMemoryUsageUnderLoad:
    ...
    def test_memory_usage_large_dataset_processing(
        self, performance_settings, large_dataset_factory, memory_monitor
    ):
        ...
        print(
            f"{'Size':<8} {'Before(MB)':<12} {'After(MB)':<12} {'Increase(MB)':<14} {'Per Item(KB)':<12}"
        )
        print("-" * 65)

        for measurement in memory_measurements:
            print(
                f"{measurement['dataset_size']:<8} "
        ...
        for measurement in memory_measurements:
            ...
            assert (
                measurement["memory_increase"] < 500
            ), f"Memory increase too high: {measurement['memory_increase']:.1f}MB for {measurement['dataset_size']} items"

            # Memory per item should be reasonable
            assert (
                measurement["memory_per_item"] < 50
            ), f"Memory per item too high: {measurement['memory_per_item']:.1f}KB"
        ...
        size_ratio = (
            largest_measurement["dataset_size"] / smallest_measurement["dataset_size"]
        )
        memory_ratio = largest_measurement["memory_increase"] / max(
            smallest_measurement["memory_increase"], 1
        )

        # Memory usage should not grow much faster than dataset size
        assert (
            memory_ratio <= size_ratio * 2
        ), f"Memory usage grows too quickly: {memory_ratio:.2f}x vs {size_ratio:.2f}x data size"
    ...
    def test_memory_leak_detection(self, performance_settings, memory_monitor):
        ...
        assert (
            growth_trend < 20
        ), f"Memory growth trend indicates possible leak: {growth_trend:.1f}MB"

    def test_memory_usage_under_concurrent_load(
        self, performance_settings, memory_monitor
    ):
        ...
        assert (
            peak_increase < 200
        ), f"Peak memory increase too high: {peak_increase:.1f}MB"
        assert (
            final_increase < 50
        ), f"Final memory increase too high (possible leak): {final_increase:.1f}MB"
