...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "memory: marks tests that monitor memory usage"
    )
    config.addinivalue_line(
...
def performance_settings(performance_database):
    ...
    cleanup_vars = [
        ...
        'ARGUS_LOG_LEVEL', 'ARGUS_SCAN_TIMEOUT', 'ARGUS_MAX_PARALLEL_SCANS',
        'ARGUS_ENABLE_ALERTS'
    ]
...
def large_dataset_factory():
    ...
    def create_devices(count=1000):
        ...
        for i in range(count):
            device_data = {
                ...
                ] if i % 3 == 0 else [{'port': 22, 'service': 'ssh'}]
            }
    ...
    def create_vulnerabilities(device_count=100, vulns_per_device=5):
        ...
        vulnerabilities = []
        cve_templates = [
            'CVE-2023-{:05d}', 'CVE-2022-{:05d}', 'CVE-2021-{:05d}'
        ]
    ...
    return {
        ...
        'vulnerabilities': create_vulnerabilities
    }
