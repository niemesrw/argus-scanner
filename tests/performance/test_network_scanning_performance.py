...
class TestNetworkScanningPerformance:
    ...
    def test_small_network_scan_performance(
        self, performance_settings, performance_timer, memory_monitor
    ):
        ...
        with patch.object(discovery.nm, "scan") as mock_scan, patch.object(
            ...
            assert (
                scan_time < 5.0
            ), f"Small network scan took too long: {scan_time:.2f}s"
            assert (
    ...
    def test_medium_network_scan_performance(
        self, performance_settings, performance_timer, memory_monitor
    ):
        ...
        with patch.object(discovery.nm, "scan") as mock_scan, patch.object(
            ...
            with patch.object(discovery, "_extract_device_info") as mock_extract:
                ...
                assert (
                    scan_time < 30.0
                ), f"Medium network scan took too long: {scan_time:.2f}s"
                assert (
    ...
    def test_large_network_scan_performance(
        self, performance_settings, performance_timer, memory_monitor
    ):
        ...
        with patch.object(discovery.nm, "scan") as mock_scan, patch.object(
            ...
            mock_hosts.return_value = large_host_list[
                :100
            ...
            with patch.object(discovery, "_extract_device_info") as mock_extract:
                ...
                assert (
                    scan_time < 60.0
                ), f"Large network scan took too long: {scan_time:.2f}s"
                assert (
    ...
    def test_vulnerability_scan_performance(
        ...
        with patch.object(vuln_scanner, "_query_cve_database") as mock_cve:
            ...
            assert (
                scan_time < 30.0
            ), f"Vulnerability scanning took too long: {scan_time:.2f}s"
    ...
    def test_port_scan_performance(self, performance_settings, performance_timer):
        ...
        with patch.object(discovery.nm, "scan") as mock_scan:
            ...
            with patch.object(discovery.nm, "__getitem__") as mock_host_data:
                ...
                result = discovery.scan_device_ports(
                    "192.168.1.100", list(range(1, 1001))
                )
    ...
    def test_scan_throughput_benchmark(
        self, performance_settings, performance_timer, network_range_generator
    ):
        ...
        for size in test_sizes:
            ...
            expected_devices = {"small": 16, "medium": 256, "large": 1024}
            ...
            with patch.object(discovery.nm, "scan") as mock_scan, patch.object(
                ...
                if size == "small":
                    mock_hosts.return_value = [
                        f"192.168.1.{i}" for i in range(1, device_count + 1)
                    ]
                else:
                    mock_hosts.return_value = [
                        f"192.168.0.{i}" for i in range(1, device_count + 1)
                    ]
        ...
        for result in results:
            print(
                f"{result['size']:<10} {result['devices']:<10} {result['time']:<10.3f} {result['throughput']:<12.1f}"
            )
        ...
        for result in results:
            ...
            ), f"Throughput too low for {result['size']}: {result['throughput']:.1f} dev/s"
